...

		{
				...
				if ((PINB & (1 << 4)) == 0)
				{
					...
					case 3:
						...
						else
						{
							...
							for (int i = 0; i <= iter; i++) // It goes here, and does a for loop with each of the instructions here
							{
			...
			if (right_button_pressed == 1)
			{
				...
				else
				{
					...
					{
						go(commands[i][0], commands[i][1], commands[i][2]);
						// Also here , will do the commands
					}
					done = 1;
				}
			}

			_delay_us(1000); // really high delay so i dont have to worry about debouncing the buttons
		}
	}
	return 0;
}